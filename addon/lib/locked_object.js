...
export function apply(AutomaticDictionary) {
    AutomaticDictionary.Lib.LockedObject = function(storeKey,storage,options,instance){
        ...
        function async sincronized( func, force ){
            if( force ) logger.warn("sincronized with force");
            try{
                var lock = await params.storage.get( lock_key );
                logger.debug("lock is " + lock);
                var nestedSync = await gotLock();
                if( force || nestedSync ||
                    ((typeof lock) === "undefined") || lock === ""  ){
                    //No lock is set so we set ours.
                    await params.storage.set( lock_key, locker_id );
                    //Check if someone else has got the lock meanwhile.
                    if( force || await gotLock() ){
                        logger.debug("executing on sync");
                        func();
                        if( !nestedSync ){
                            await params.storage.set( lock_key, "" ); //Release lock
                        ...
                        logger.warn("lock is not its own: " +
                                    await params.storage.get( lock_key ) + " != " + locker_id);
                    }
                }else{
                ...
                await params.storage.set( lock_key, "" ); //Release the lock
                throw e;
            }
        ...
        function async gotLock(){
          return (""+ (await params.storage.get( lock_key ))) === locker_id;
        }
